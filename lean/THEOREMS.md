...
|---|---|---|---|
| `bdiff_smul` | `bdiff` is homogeneous: scalars pass through. | ℂ floor | — |
| `A4_of_A1` | A1 → A4. | ℂ floor | — |
| `B5_of_A4_B4` | A4 ∧ B4 → B5. | ℂ floor | — |
| `C2_of_C1` | C1 → C2. | ℂ floor | — |
| `C3_of_A4_C2` | A4 ∧ C2 → C3. | ℂ floor | — |
| `C3lower_of_A4_C2` | A4 ∧ C2 → C3 (decay half). | ℂ floor | — |
| `A2` | A2, proved rather than assumed, from `EulerFactorChain.euler_product_riemannZeta` (Mathlib's `riemannZeta_eulerProduct_tprod`). | ℂ floor | — |
| `A3` | A3, proved rather than assumed. | ℂ floor | — |
| `C1` | C1, proved rather than assumed. | ℂ floor | — |
| `C2` | C2, now unconditional. | ℂ floor | — |
| `A1` | A1, proved rather than assumed, from `EulerFactorChain.symbol_of_backward_difference`. | ℂ floor | — |
| `A4` | A4, now unconditional. | ℂ floor | — |
| `B4` | B4, proved rather than assumed, from `EulerFactorChain.h_eq_gain_pow_on_critical_line`. | ℂ floor | — |
| `B5` | B5, now unconditional. | ℂ floor | — |
| `C3` | C3, now unconditional. | ℂ floor | — |
| `C3lower` | C3's decay half, unconditional. | ℂ floor | — |
| `tableFrom_eq_bdiff_iter` | The weld. | ℂ floor | `lab_notebook_2.md` |
| `tableFrom_mode` | The chain, welded. | ℂ floor | `lab_notebook_2.md` |
| `tableFrom_norm_on_critical_line` | The table on the critical line. | ℂ floor | — |
...
| `gain_sq_periodic` | The circle. | ℂ floor | `Euler-Factor-Chain.md` |
| `period_vacuous_at_one` | The trap `gain_sq_periodic`'s `b ≠ 1` closes: at `b = 1` the period is zero and the statement holds for any function whatsoever. | ℂ floor | — |
| `gain_sq_at_floor` | D1, floor half. | ℂ floor | `Euler-Factor-Chain.md` |
| `gain_sq_at_ceiling` | D1, ceiling half. | ℂ floor | `Euler-Factor-Chain.md` |
| `C2_floor_attained` | D2. | ℂ floor | — |
| `C2_ceiling_attained` | C2's upper bound is ATTAINED. | ℂ floor | — |
| `ceiling_base` | D4. | ℂ floor | `Euler-Factor-Chain.md` |
| `ceiling_dominates_floor` | D3. | ℂ floor | `Euler-Factor-Chain.md`, `The-Deep-Ladder.md` |
| `joint_gain_periodic_of_commensurate` | The collapse. | ℂ floor | — |
| `second_ladder_winds_densely` | The torus. | ℂ floor | — |
...
| `unique_of_isTableOf` | Uniqueness. | **none** | `README.md`, `The-Four-Zeros.md` |
| `eq_of_same_row` | Two tables over the same row agree everywhere. | **none** | `The-Four-Zeros.md` |
| `zero_determined_by_row` | A zero is determined by the row. | propext | `lab_notebook_2.md` |
| `tableFrom_add` | Differencing is linear, at every depth. | propext | `lab_notebook_2.md` |
| `tableFrom_smul` |  | propext | — |

## Covering (6)

| theorem | claim | axioms | cited by |
|---|---|---|---|
| `exists_near_lattice` | The covering lemma. | ℂ floor | — |
| `covered_of_half_spacing` | Vacuity. | ℂ floor | — |
| `covered_smul` | Selectivity is a ratio. | ℂ floor | — |
| `covering_not_monotone` | Non-monotone by construction. | ℂ floor | — |
...
| `count_does_not_determine_spread` | The coefficient-count account fails. | ℂ floor | — |
...
| theorem | claim | axioms | cited by |
|---|---|---|---|
| `symbol_of_backward_difference` | A1. | ℂ floor | `lab_notebook_2.md` |
| `diagonal_gain` | The diagonal gain. | ℂ floor | — |
| `diagonal_cell` | The cell value along a diagonal in closed form: with `r = d + c`, the mode's `b^(rρ)·(sym b ρ)^d` collapses to `b^(cρ)·(b^ρ − 1)^d`. | ℂ floor | — |
| `diagonal_over_column` | dia/col = `b^ρ`. | ℂ floor | — |
| `diagonal_over_column_at_half` | At `ρ = 1/2` that ratio is `√b`. | ℂ floor | — |
| `symbol_at_one` | A1 at `ρ = 1`. | ℂ floor | — |
| `euler_product_riemannZeta` | A2. | ℂ floor | `The-Deep-Ladder.md` |
...
| `euler_product_sym` | A3, the single-base reading with the `b` index retained. | ℂ floor | — |
...
| `no_interior_peak` | No power-law tradeoff of this form has an interior peak. | ℂ floor | — |
...
| theorem | claim | axioms | cited by |
|---|---|---|---|
| `rowOf_eq_rowN` | The bridge: composing `P` with `b^·` turns one into the other. | propext | — |
...
| `row_two_eq_pair` | Base 4's row is base 2's row summed in pairs. | propext, Quot.sound | — |
| `row_three_eq_triple` | Base 8's row is base 2's row summed in triples. | propext, Quot.sound | — |
| `rowN_comp` | Decimation composes. | propext | — |
| `dyadicRow_eq_rowN` | The weld to the measured row. | propext, Quot.sound | — |
| `measured_row_four` | The measured base-4 row, from the 21 pinned values and nothing else. | **none** | — |
| `measured_row_eight` | The measured base-8 row, same 21 values read at decimation 3. | **none** | — |
...
| theorem | claim | axioms | cited by |
|---|---|---|---|
| `backward_difference_pow` | The same step inside ℤ, where the table lives: `(1 − b⁻¹)·b^(r) = (b−1)·b^(r−1)`. | propext | — |
...
| `pair_identity` | I1, the pair identity. | propext, Quot.sound | `README.md`, `The-Composite-Arm.md`, `The-Fold.md` +2 |
| `composite_of_prime_zero` | I5, the pole. | propext, Quot.sound | — |
| `coeff_eq_one_iff_base_two` | The corollary. | propext, Quot.sound | `lab_notebook_2.md` |
| `total_eq_pow_iff_base_two` | The corollary, in the identity's own terms. | propext, Quot.sound | — |
| `base_three_carries_factor` | Base three does carry the factor: the total at `(r,d)` is `2^(d+1)·3^(r−1−d)`, never a bare power of three. | **none** | — |
| `base_four_carries_factor` | Base four likewise: `3^(d+1)·4^(r−1−d)`. | **none** | — |
...
| `boundary_can_move` | `r − d > R` cannot be relaxed to `r − d ≥ R`. | propext, Quot.sound | — |
...
| theorem | claim | axioms | cited by |
|---|---|---|---|
| `norm_zmap` | The map. | ℂ floor | `lab_notebook_2.md` |
| `norm_zmap_critical` | The critical line lands on the circle of radius `b^(−1/2)`. | ℂ floor | `Euler-Factor-Chain.md` |
| `zmap_shift` | The deck transformation. | ℂ floor | `Euler-Factor-Chain.md` |
| `zmap_period` | The other generator. | ℂ floor | `Euler-Factor-Chain.md` |
| `zmap_functional_equation` | The functional equation, in `z`. | ℂ floor | `Euler-Factor-Chain.md`, `lab_notebook_2.md` |
| `annulus_modulus` | G7's modulus. | ℂ floor | `Euler-Factor-Chain.md`, `lab_notebook_2.md` |
| `zmap_period_tau` | `τ` is exactly the period `zmap_period` uses. | ℂ floor | `lab_notebook_2.md` |
| `tau_pow` | The power chain. | ℂ floor | — |
...
| `gens_linearIndependent` | `generators_indep` in the form Mathlib's basis constructor wants. | ℂ floor | — |
| `periodLattice_eq_span` |  | ℂ floor | — |
| `periodLattice_discrete` | The lattice is discrete. | ℂ floor | — |
...
| `rpow_left_inj` | For `b > 1` the real power is injective in the exponent. | ℂ floor | — |
| `zmap_ne_zero` | The image of `s` under the `z`-map is never zero. | ℂ floor | — |
| `on_critical_line_iff_norm` | The critical line is exactly the fixed circle. | ℂ floor | — |
| `on_critical_line_iff_inversion_fixed` | RH's condition, as a fixed point of the inversion. | ℂ floor | — |
| `riemannHypothesis_iff_zeros_inversion_fixed` | The Riemann hypothesis, restated on the torus. | ℂ floor | — |
| `norm_zmap_zero_line` | `Re s = 0` lands on the unit circle. | ℂ floor | — |
| `strip_is_fundamental_domain` | The critical strip is one fundamental domain of `ℂ*/b^ℤ`. | ℂ floor | — |
| `zeros_re_lt_one` | Every zero of ζ has `Re s < 1`, from Mathlib's non-vanishing on the closed half-plane `1 ≤ Re s`. | ℂ floor | — |
| `zeros_outside_inner_circle` | Every zero lies strictly outside the inner circle. | ℂ floor | — |
| `zeros_re_pos` | Every nontrivial zero has `0 < Re s`. | ℂ floor | — |
| `zeros_in_fundamental_annulus` | Every nontrivial zero lies in the open fundamental annulus. | ℂ floor | `README.md` |
| `riemannHypothesis_iff_zeros_on_middle_circle` | RH, as a statement about one annulus. | ℂ floor | — |
| `zmap_pair_product` | The pair identity. | ℂ floor | `NOTEPAD.md`, `lab_notebook_2.md` |
| `pair_collapses_iff_critical` | The pair collapses exactly on the critical line. | ℂ floor | — |
| `riemannHypothesis_iff_pair_collapses` | RH: the pair collapses at every zero. | ℂ floor | — |
| `sym_zero_on_outer_circle` | A zero of the table's symbol lands on `|z| = 1`. | ℂ floor | — |
| `sym_zero_partner_on_inner_circle` | Its inversion partner lands on `|z| = b^(−1)`. | ℂ floor | — |
| `critical_circle_is_lattice_inversion_mean` | The table's lattice, inverted, gives the critical circle. | ℂ floor | `README.md` |

## TwinLattice (3)

| theorem | claim | axioms | cited by |
|---|---|---|---|
| `twin_lower_mod_six` | The lattice. | propext, Quot.sound | `The-Twin-Lattice.md` |
| `twin_pocket` | The pocket. | propext, Quot.sound | `The-Twin-Lattice.md` |
...
| `zero_iff_repeat` | A zero is exactly a repeat one depth up. | propext | `FORMAT.md`, `README.md`, `The-Fold.md` +3 |
| `neg_below_zero` | A zero puts its left neighbour, negated, directly beneath it. | propext, Quot.sound | — |
| `pair_shares_diagonal` | And those two cells share a diagonal. | propext, Quot.sound | — |
| `tableFrom_eq_fwdDiff` | The table's `d`-fold backward difference is `(-1)^d` times Mathlib's `d`-fold forward difference at step `-1`. | propext, Quot.sound | — |
| `tableFrom_eq_stencil` | The operator IS Pascal. | ℂ floor | `README.md` |
...
| `stencil_eq_wings` | The fold is an identity, not a test. | ℂ floor | — |
| `stencil_eq_zero_iff_wings` | A zero is where the wings balance. | ℂ floor | — |
| `tableFrom_eq_zero_iff_wings` | The cell, folded. | ℂ floor | — |
| `repeat_iff_wings` | Two readings of one fact. | ℂ floor | — |
| `stencil_weights_antisymm` | The stencil is antisymmetric when its order is odd. | ℂ floor | — |
| `stencil_arms_eq` | The two arms weigh the same. | ℂ floor | — |
| `stencil_arm_doubled` | And each arm is `2^(N-1)`. | ℂ floor | — |
| `window_exclusive_of_prime_exponent` | The (20,6) window is base-2 exclusive. | ℂ floor | `Commensurate-Ladders.md`, `The-Fold.md`, `lab_notebook_2.md` |
| `window_shared_of_composite_exponent` | The (8,3) window is not. | propext | — |
| `base_of_meets_two` | Only powers of two reach a dyadic window. | ℂ floor | `lab_notebook_2.md` |
| `factorization_proportional` | Two ladders that meet have proportional exponents. | ℂ floor | — |
