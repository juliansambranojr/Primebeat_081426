...
lake build             # compiles all 24 modules
```
...
Expected: compiles clean, **8050 jobs**, zero `sorry`, **282 theorems**. A few
...
* **`grind` is the one expensive tactic you can actually reach here, and it
  is the most expensive of them.** Probed on this toolchain against core with
  no Mathlib on the path:
  ...
  So three of the four tactics a Mathlib-trained reflex reaches for cannot be
  typed in a Mathlib-free module — they fail loudly and cost nothing. `grind`
  compiles, looks like a win, and quietly pulls in `Classical.choice`, which no
  ℤ or ℕ statement in this tree otherwise needs. It is the only silent one.
  `PairIdentity.lean`:56-57 records the same finding at the module level.
* **A named Mathlib lemma can be worse than a tactic** — this one applies when
  Mathlib *is* imported, since neither name below exists without it. Replacing
  `ring` with `mul_sub` in `tableFrom_smul` *raised* the axiom count to include
...
`Construction`'s theorems at their reduced counts. Six of the 24 modules are now
...
Of the 201 theorems at `Classical.choice`, the ones mentioning ℝ or ℂ can never
...
24 modules. Every theorem carries a `#guard_msgs`-pinned `#print axioms`,
282 of them, so `lake build` fails the moment a proof starts depending on
...
Zeros              a zero is a repeat; the four computed zeros and their two
                   non-zero neighbours; the measured-zero results — NO MATHLIB
ZerosStencil       the Mathlib-dependent half of Zeros — the stencil family and
                   the four ladder results; namespace is still `Zeros`
ZeroCells          the four zeros are one object with four names; three
                   transcriptions welded to the computed list — NO MATHLIB
...
SeedPerturbation   a convention change cannot reach a cell with r − d > R_e
                   — NO MATHLIB
Propagation        the recurrence as transport: first-order upwind, one rung
                   per depth step, speed exactly 1 — NO MATHLIB
Superposition      the depth gain extends from one mode to a finite sum
TransferOp         bdiff named as the transfer operator of the ladder shift,
...
NyquistPeak        the difference filter's gain is stationary AND maximal at
                   b* = exp(pi/gamma) — the mechanism behind a locked prereg
Nonvanishing       O67's conditional arrow in the house pattern: two analytic
                   inputs and one gap condition, with the joins proved
MainTerm           stage 2 — the calculus discharging hM, the iterated MVT
                   replaced by unit-step monotonicity
Expansion          stage 2b — the derivative of 2^x/x explicitly, and its floor
Schoenfeld         stage 3's interface: Schoenfeld 1976 Corollary 1 verbatim
```

Six of the 24 are Mathlib-free, marked above and closed transitively.
Theorem-level detail is not repeated here: `lean/THEOREMS.md`, generated by
`utilities/theorem_index.py`, indexes all 282 with the claim each one makes and
its axiom pin. Re-run that script after any change to `lean/`.
