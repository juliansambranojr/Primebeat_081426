...
so the closed unit ball maps onto `|s - (2+iT)| ≤ 2`, `r = 3/4` covers
`|s - (2+iT)| ≤ 3/2` (which contains `1/2 + iT`), and `R = 7/8` needs a
majorant on `|s - (2+iT)| ≤ 7/4`, i.e. `Re ∈ [1/4, 15/4]`. Three obligations
follow, and all three are proved here rather than imported:

  `zeta_disk_upper`    `‖ζ w‖ ≤ 14 T` on that disk, from upstream
...
CONSTANTS. `1/log(7/6) ≤ 7` (from `log(6/7) ≤ -1/7`) and `log 42 ≤ 4.16`
(from `42 ≤ 2^6` and `log 2 < 0.6931471808`) give

    zetaLocalCount T ≤ 7 log T + 30    for all T ≥ 2.

Crude-explicit is the spec. Entry 130's budget accepts `A₁ ≤ 100`,
`A₃ ≤ 1000`, so these clear it by an order of magnitude.
...
theorem zeta_disk_upper {T : ℝ} (hT : 2 ≤ T) {w : ℂ}
  ...
  have hnormw : ‖w‖ ≤ T + 15 / 4 := by
    ...
    calc ‖w‖ = ‖(w - (2 + Complex.I * (T : ℂ))) + (2 + Complex.I * (T : ℂ))‖ := by ring_nf
      ...
      _ ≤ 7 / 4 + (2 + T) := by linarith
      _ = T + 15 / 4 := by ring
  have leadingTerms : ‖(1 : ℂ) + 1 / (w - 1)‖ ≤ 5 := by
    have h4 : ‖(1 : ℂ) / (w - 1)‖ ≤ 4 := by
      rw [norm_div, norm_one, div_le_iff₀ (by linarith : (0:ℝ) < ‖w - 1‖)]
      linarith
    calc ‖(1 : ℂ) + 1 / (w - 1)‖ ≤ ‖(1 : ℂ)‖ + ‖(1 : ℂ) / (w - 1)‖ := norm_add_le _ _
      _ ≤ 1 + 4 := by rw [norm_one]; linarith
      _ = 5 := by norm_num
...
/-- **The window.** `ζ`'s zeros in the closed disk of radius `3/2` about
`2 + iT`; this disk contains the critical-line point `1/2 + iT`. -/
def zetaWindow (T : ℝ) : Set ℂ :=
  {ρ : ℂ | ζ ρ = 0 ∧ ‖ρ - (2 + Complex.I * (T : ℂ))‖ ≤ 3 / 2}
...
theorem jensenF_bound {T : ℝ} (hT : 2 ≤ T) {z : ℂ} (hz : ‖z‖ ≤ 7 / 8) :
    ‖jensenF T z‖ ≤ 42 * T := by
  have hnum : ‖ζ (2 * z + (2 + Complex.I * (T : ℂ)))‖ ≤ 14 * T := by
  ...
  calc ‖ζ (2 * z + (2 + Complex.I * (T : ℂ)))‖ ≤ 14 * T := hnum
    _ = 42 * T * (1 / 3) := by ring
    _ ≤ 42 * T * ‖ζ (2 + Complex.I * (T : ℂ))‖ :=
...
of `ζ` in the closed disk `|s - (2 + iT)| ≤ 3/2` — a disk which contains the
critical-line point `1/2 + iT` — is at most `7 log T + 30`.

Backlund's Jensen step, with crude-explicit constants: upstream `ZerosBound`
at `r = 3/4`, `R = 7/8` on `f z = ζ (2z + 2 + iT) / ζ (2 + iT)`. -/
theorem zeta_local_zero_count {T : ℝ} (hT : 2 ≤ T) :
    ∑ ρ ∈ (zetaWindow_finite hT).toFinset, (analyticOrderNatAt ζ ρ : ℝ)
      ≤ 7 * Real.log T + 30 := by
  have hrlt1 : (3 : ℝ) / 4 < 1 := by norm_num
  have hZB := ZerosBound (B := 42 * T) (r := 3 / 4) (R := 7 / 8) (f := jensenF T)
  ...
  rw [Nat.cast_sum, show (7 : ℝ) / 8 / (3 / 4) = 7 / 6 by norm_num] at hZB
  ...
  have hlog76 : (1 : ℝ) / 7 ≤ Real.log (7 / 6) := by
    ...
    rw [show (7 : ℝ) / 6 = ((6 : ℝ) / 7)⁻¹ by norm_num, Real.log_inv]
    linarith
  have hlog76pos : (0 : ℝ) < Real.log (7 / 6) := by linarith
  have hlog42 : Real.log 42 ≤ 4.16 := by
    have h64 : Real.log 42 ≤ Real.log 64 := Real.log_le_log (by norm_num) (by norm_num)
    have h2 : Real.log 64 = 6 * Real.log 2 := by
      rw [show (64 : ℝ) = 2 ^ (6 : ℕ) by norm_num, Real.log_pow]
      norm_num
    have h3 := Real.log_two_lt_d9
    linarith
  have hlog42nn : (0 : ℝ) ≤ Real.log 42 := Real.log_nonneg (by norm_num)
  have hlogT : (0 : ℝ) ≤ Real.log T := Real.log_nonneg (by linarith)
  have hlogB : Real.log (42 * T) = Real.log 42 + Real.log T :=
    Real.log_mul (by norm_num) (by linarith)
  have hinv : 1 / Real.log (7 / 6) ≤ 7 := by
    rw [div_le_iff₀ hlog76pos]
    linarith
  calc ∑ z ∈ (finiteSetOfZeros_mono hrlt1 (jensenF_zeros_finite hT)).toFinset,
        (analyticOrderNatAt (jensenF T) z : ℝ)
      ≤ 1 / Real.log (7 / 6) * Real.log (42 * T) := hZB
    _ ≤ 7 * Real.log (42 * T) := by
        refine mul_le_mul_of_nonneg_right hinv ?_
        rw [hlogB]; linarith
    _ = 7 * (Real.log 42 + Real.log T) := by rw [hlogB]
    _ ≤ 7 * Real.log T + 30 := by linarith

open Classical in
/-- **The count function.** Total zero order of `ζ` in `|s - (2+iT)| ≤ 3/2`. -/
...
zero count with the explicit constants `A₁ = 7`, `A₃ = 30`. Entry 130's
budget accepts `A₁ ≤ 100`, `A₃ ≤ 1000`. -/
theorem localCount_holds : StmtLocalCount zetaLocalCount 7 30 := by
  intro T hT
