...
**Provenance.** Julian switched the orchestrator to `Opus 5` after the sibling repo landed, for his weekly limit, and back to `Fable 5.1` before this audit. The commit trailers track it: every commit from block 13h through unit `unit_window` 0360 carries `Claude Opus 5 (1M context)`, and every commit from block `13j` on carries `Claude Fable 5.1`. The builder in every relay run was Opus. So the two deaths this unit corrects were `Opus 5`'s orchestration, and the loop on the builder's side ran the same model throughout.
