...
def main():
    ...
    check1 = []             # record of CHECK 1 for the results JSON
    for d in depths[:-1]:
        rats = []
        ...
        if rats:
            ...
            check1.append({"depth": d, "median_observed_ratio": m,
                           "ratio_to_predicted": m / g})
...
""")
    ...
    if not args.no_json:
        ...
        payload = {
            ...
            "constants": {
                "GAMMA_1": GAMMA_1,
                "alpha_reference": 0.4334,
                "rh_value": 0.5000,
                "rng_seed": 2026,
            },
            "summary": {
                "omega": omega,
                "comb_gain_single_step": g,
                "slope_comb_corrected": coef[1],
                "r2": r2,
                "median_local_beta": np.median(beta_loc),
                "mean_local_beta": beta_loc.mean(),
                "slope_upper_half": coef2[1],
                "upper_half_rmin": int(rs[half]),
                "synthetic_recovered_mean": recov.mean(),
                "synthetic_recovered_sd": recov.std(),
                "pipeline_bias": bias,
                "recovery_p5": np.percentile(recov, 5),
                "recovery_p95": np.percentile(recov, 95),
                "beta_bias_corrected": corrected,
                "n_valid_trials": len(recov),
                "check1": check1,
            },
            "rows": [
                {"r": rs[i], "dtilde": tilde[i],
                 "local_beta": beta_loc[i] if i < len(beta_loc) else None}
                for i in range(len(rs))
            ],
        }
