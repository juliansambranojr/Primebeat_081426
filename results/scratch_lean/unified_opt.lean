/-
UNIFIED-OPT — the substitute chain with BOTH constant levers applied.

LEVER 1 (new, `Slice5`): `‖ζ'/ζ(s)‖ ≤ 2/(σ-1) + 4/(σ-1)²` for `Re s > 1`, from
the absolutely convergent von-Mangoldt Dirichlet series.  RH-free, contour-free.
At `σ₀ = 1 + 1/L` this is `2L + 4L²`, replacing the critical-line bound the
`I₁`/`I₉` integrals had been borrowing (≈ 5.4×10⁴ at the census floor).

LEVER 2 (radii): slices 3 and 4 re-run at `r' = 3/4, r = 29/32, R' = 91/100,
R = 15/16`.  `r' = 3/4` is a hard floor (‖z₀‖ = (2-σ₁)/2 → 3/4), `R = 15/16`
is `Stage3.jensenF_bound`'s proved reach; `r` and `R'` are free.  FinalBound
constant 6600 → 4060, ZerosBound constant 15 → 30.
...
/-- LEVER 2 — the FinalBound constant at `r' = 3/4, r = 29/32, R' = 91/100, R = 15/16`. -/
theorem finalBoundConst_le :
    16 * ((29:ℝ)/32) ^ 2 / ((29/32) - (3/4)) ^ 3
      + 1 / (((15/16:ℝ) ^ 2 / (91/100) - 91/100) * Real.log ((15/16)/(91/100))) ≤ 4060 := by
  have hlog : (11:ℝ)/375 ≤ Real.log (375/364) := by
    have hh1 : Real.log (364/375) ≤ 364/375 - 1 := Real.log_le_sub_one_of_pos (by norm_num)
    rw [show (375:ℝ)/364 = ((364:ℝ)/375)⁻¹ by norm_num, Real.log_inv]
    linarith
  ...
  have h2 : 1 / (((15/16:ℝ) ^ 2 / (91/100) - 91/100) * Real.log ((15/16)/(91/100)))
    ...
    calc 1 / ((8129/145600:ℝ) * Real.log ((15/16)/(91/100)))
        ≤ 1 / ((89419:ℝ)/54600000) := one_div_le_one_div_of_le hpos hle
...
/-- LEVER 2 — the FinalBound constant at `r' = 3/4, r = 29/32, R' = 91/100, R = 15/16` — the
...
theorem logDerivZeta_compact (hRH : RiemannHypothesis) {t σ₁ : ℝ} (ht : |t| ≤ 2)
  ...
  have hmem : z0 ∈ Metric.closedBall (0:ℂ) (3/4) \ SetOfZeros (91/100) f := by
    ...
    · intro hmem2
      exact hfz0ne hmem2.2
  have hFB := FinalBound (B := 1300) (r' := 3/4) (r := 29/32) (R' := 91/100) (R := 15/16)
    (f := f) (z := z0) hBgt (by norm_num) (by norm_num) hr1 (by norm_num) (by norm_num)
    (by norm_num) hana hf0 hfin hbd hmem
  have hZB := ZerosBound (B := 1300) (r := 29/32) (R := 15/16) (f := f)
    (by norm_num) hr1 (by norm_num) (by norm_num) hana hf0 hfin hbd
  set S := (finiteSetOfZeros_mono hr1 hfin).toFinset with hS
  ...
  have hcount : ((∑ ρ ∈ S, analyticOrderNatAt f ρ : ℕ) : ℝ) ≤ 229 := by
    have hlog1514 : (1 : ℝ) / 30 ≤ Real.log (30 / 29) := by
      have h1 : Real.log (29 / 30) ≤ 29 / 30 - 1 := Real.log_le_sub_one_of_pos (by norm_num)
      rw [show (30 : ℝ) / 29 = ((29 : ℝ) / 30)⁻¹ by norm_num, Real.log_inv]
      linarith
    ...
    rw [hS]
    calc ((∑ ρ ∈ (finiteSetOfZeros_mono hr1 hfin).toFinset, analyticOrderNatAt f ρ : ℕ) : ℝ)
        ≤ 1 / Real.log ((15/16) / (29/32)) * Real.log 1300 := hZB
      _ = 1 / Real.log (30/29) * Real.log 1300 := by norm_num
      _ ≤ 30 * Real.log 1300 := mul_le_mul_of_nonneg_right hinv hlogBnn
      _ ≤ 229 := by linarith
  have hK := finalBoundConst_le
  have hstep : ‖deriv f z0 / f z0‖ ≤ 30960 + (2 / (σ₁ - 1/2)) * 229 := by
    have htri : ‖deriv f z0 / f z0‖
        ≤ ‖deriv f z0 / f z0 - ∑ ρ ∈ S, (analyticOrderNatAt f ρ : ℂ) / (z0 - ρ)‖
          + ‖∑ ρ ∈ S, (analyticOrderNatAt f ρ : ℂ) / (z0 - ρ)‖ := by
      simpa using norm_add_le (deriv f z0 / f z0
    ...
    have hA : ‖deriv f z0 / f z0 - ∑ ρ ∈ S, (analyticOrderNatAt f ρ : ℂ) / (z0 - ρ)‖
        ≤ 30960 := by
      refine le_trans hFB ?_
      calc (16 * ((29:ℝ)/32) ^ 2 / ((29/32) - (3/4)) ^ 3
            + 1 / (((15/16:ℝ) ^ 2 / (91/100) - 91/100) * Real.log ((15/16)/(91/100))))
            * Real.log 1300
          ≤ 4060 * Real.log 1300 := mul_le_mul_of_nonneg_right hK hlogBnn
        _ ≤ 30960 := by linarith
    have hB2 : ‖∑ ρ ∈ S, (analyticOrderNatAt f ρ : ℂ) / (z0 - ρ)‖
        ≤ (2 / (σ₁ - 1/2)) * 229 := by
      refine le_trans hsum ?_
  ...
  have hexp : ‖2 * ((1:ℂ) / (s - 1) + deriv ζ s / ζ s)‖
    ...
    simp only [Complex.norm_ofNat]
    linarith
  have hfinal : (2 / (σ₁ - 1/2)) * 229 = 458 / (σ₁ - 1/2) := by ring
  rw [hfinal] at hstep
  have h115 : (229:ℝ) / (σ₁ - 1/2) = (458 / (σ₁ - 1/2))/2 := by ring
  rw [h115]
  linarith
...
theorem logDerivZeta_line (hRH : RiemannHypothesis) {X t : ℝ}
  ...
  · have h4 := Slice4.logDerivZeta_compact hRH hsmall hσlo hσhi
    ...
    rw [hd]
    have hdd : (229:ℝ) / (1 / L) = 229 * L := by field_simp
    rw [hdd]
