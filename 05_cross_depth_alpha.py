...
def main():
    ...
    obs = []
    per_depth = []          # parallel record for the results JSON; must be
                            # built here, before obs is rebound to a filtered
                            # array (which destroys depth alignment)
    for d in depths:
        rs, vals = [], []
        ...
        obs.append(a)
        per_depth.append({"depth": d, "alpha": a, "r2": r2, "n_points": n})
...
""")
    ...
    if not args.no_json:
        ...
        payload = {
            ...
            "summary": {
                "mean_alpha": obs.mean(),
                "observed_spread": obs.std(),
                "alpha_min": obs.min(),
                "alpha_max": obs.max(),
                "n_depths_fitted": len(obs),
                "synthetic_spread_mean": spreads.mean(),
                "synthetic_spread_p5": np.percentile(spreads, 5),
                "synthetic_spread_p95": np.percentile(spreads, 95),
                "synthetic_recovered_mean_alpha": means.mean(),
                "fitting_bias": bias,
                "p_synthetic_ge_observed": p_hi,
                "n_valid_trials": len(spreads),
                "per_depth": per_depth,
            },
        }
