...
- [open]   2026-08-19  figures/coverage.png was committed at 3da2ee8 with NO script — its analysis was inline too — and is reconstructed as analysis/2026-08-19_table_structure/scripts/t15_cell_coverage.py, which POSTDATES the result it reproduces just as t9-t14 do; every per-base mean, zero-mean and z REPRODUCED to printed precision, with ONE disagreement — base 6's per-zero counts are [0, 1, 2, 2], not the reported [0, 1, 1, 3], and [0, 1, 1, 3] is base 7's list, so a TRANSCRIPTION SLIP and not a computational difference; both sum to 5, so the mean 1.25 and the z -1.04 were unaffected, which is why it went unnoticed; THE KILL REPRODUCES — maximum distinct coverage values at any fixed depth is 2, across all 224 depth-base pairs, because the window's width in b-rungs is (d+1)*ln2/ln b, a function of d ALONE; CORROBORATION — the zeros' mean depth has z = -0.99, the same ~ -1.0 the coverage z gives at every base, so coverage's z IS the depth z
- [open]   2026-08-19  six analyses reported today were run INLINE as heredoc commands with no script saved, so the results predate any reproducible instrument — reconstructed as analysis/2026-08-19_table_structure/scripts/t9_subthreshold_ladder.py, t10_blocksum_lowpass.py, t11_decimation_alias.py, t12_chain_vs_orphan.py, t13_signflip_crossover.py, t14_s_matched_control.py and re-run; THE SCRIPTS POSTDATE THE RESULTS THEY REPRODUCE (mtimes 12:23-12:25 against a session that ended at 12:09) and are reconstructions from the reported numbers, not the code that produced them; ORDERING EVIDENCE from file mtimes — 11:01 shape32.py, 11:35 t5_2d.py/spectrum2d.png, 11:41 t6_multirate.py/multirate.png, 12:03 coverage.png (its analysis was inline too, no script survived and it is NOT among the six reconstructed), 12:06 t7_phase.py/phase.png, 12:09 t8_subzeros.py — the six inline analyses fell between those marks and their exact times are NOT RECOVERABLE, the interleaving above is the only chronology there is; WHAT REPRODUCED — t10 exact (base 4 = dyadic in pairs True, base 8 in triples True, Dirichlet 0.1853/0.2876/0.1725 at omega 2.7689, four zeros at exactly (2,1)(4,1)(8,3)(20,6) for merge k=1 and 0 for k=2..6 at 2^48), t11 exact (fold(k*parent alias) = direct alias to <=1.8e-15 for bases 4/8/16/9/27 at 0.7453/2.0236/1.4907/0.3588/2.6035, base 9 at 0.86 cycles), t12 exact (0.5197-0.5346 across bases 2-9 at 2^48, orphan mean 0.5242, chain mean 0.5321 — where 'chain' is the three bases WITH a parent, 4/8/9, not the five in any chain; 2 and 3 are roots at 0.5253), t13 exact (dyadic flip crossover d=7 matching t2's spectral 7, triadic 12 against spectral 10, bases 4-9 flat 0.00 at every depth, invariant for MIN_ROW 3..8), t14 within Monte Carlo error (observed 26.744 exact, matched null 25.724+-0.744 vs reported 25.731+-0.747 — 1.3 MC standard errors, z +1.37 vs +1.36, p 0.915 vs 0.909) and its S recomputation by the Pascal recurrence matched results/sub_integer_base_scan.json at all 121 zeros, 0 mismatches; WHAT DID NOT — t9's structure reproduced EXACTLY (rung counts 142/186/233/248/286/317/358, Nyquist 17.23/22.48/28.28/30.10/34.62/38.51/43.44, and every base recovering exactly the zeros beneath its own ceiling) but 6 of the 7 recovered gamma values differ in the third decimal — 21.021 vs 21.022, 25.018 vs 25.016, 30.448 vs 30.449, 32.927 vs 32.924, 37.644 vs 37.645, 40.934 vs 40.933, only 14.141 identical — differences up to 0.003 against a periodogram resolution element of 0.243 rad, so agreement is well inside resolution but the exact digits are NOT reproduced and the inline original must have differed in some detail (grid tested at four spacings, all give the same peaks to 0.003, so it is not the grid); ALSO UNRECOVERED — t9 finds gamma8 = 43.3271 beneath base 1.0750's Nyquist 43.44 with its peak at 43.565 ABOVE the ceiling, which the reported table did not list; nothing was tuned to close any gap
