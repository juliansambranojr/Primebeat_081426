...
def main():
    ...
    all_centres = []
    per_depth = []          # parallel record for the results JSON
    for d in depths:
        vals, rs = [], []
        ...
        per_depth.append({
            ...
            "window_slope_mean": slopes.mean(),
            "window_slope_sd": slopes.std(),
            "window_slope_last": slopes[-1],
            "beta_mean": beta.mean(),
            "beta_sd": beta.std(),
            "n_points": len(beta),
            "r_min": rs[0],
            "r_max": rs[-1],
            "n_blowups": int((np.abs(beta) > 3).sum()),
            "windowed_slopes": list(slopes),
        })
...
""")
    ...
    if not args.no_json:
        ...
        payload = {
            ...
            "constants": {
                "alpha_fit_r60": 0.4334,
                "alpha_fit_r40": 0.3500,
                "rh_value": 0.5000,
            },
            "summary": {
                "sd_median_across_depths": arr[:, 0].std(),
                "sd_trimmed_across_depths": arr[:, 1].std(),
                "sd_window_mean_across_depths": arr[:, 2].std(),
                "per_depth": per_depth,
            },
        }
