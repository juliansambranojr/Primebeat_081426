...
Exploratory analysis from 2026-08-19, copied out of a session scratchpad. **No prereg, no decision rule, no verdict** — nothing here may be cited as a verdict, and none of it is part of the O-series; the `t1`–`t21` names are the session's own, not O-numbers, and `t15` was named later by the reconstruction agent, not during the session.

Run with `.venv/bin/python`; requires `primecountpy`, `mpmath`, `numpy`, `matplotlib`. `t9`–`t14` and `t16`–`t21` are console-only and write no figure. Every script now tees its console output, so `results/` holds one `.txt` per script with no exceptions.

Outputs now land beside the scripts — figures in `figures/`, console captures in `results/` — at paths resolved by `scripts/_paths.py` from that file's own location rather than from the working directory. A run therefore needs no migration afterwards, and any script can be run from any working directory. `_paths.py` also tees a script's console output into `results/<script>.txt`; the terminal still gets everything, the file is a copy.

Ceilings differ and **results are not comparable across ceilings** — the explicit formula is asymptotic, so a taller ceiling improves agreement for reasons unrelated to the structure being measured. `t1`–`t4`, `shape32`, `t13`, `t14`, `t15` use 2³²; `t9` uses 2⁴⁴; `t4_each48`, `t5`, `t6`, `t10`, `t11`, `t12`, `t17`, `t19`, `t20` and `pages/t4_ceilings.html` use 2⁴⁸; `t18` uses 2⁴⁴ and `t21` is arithmetic on a fixed base with no ceiling. `t16` sweeps 2²⁸ through 2⁴⁸ deliberately, and is the one place a ceiling comparison is the measurement rather than a confound — the base and the sample set are held fixed and only the window is extended.
...
- `t19_diagonal.py` — the diagonal as the null direction of the cell total. Checks that prime + composite is constant along every `r − d = c` at b = 2, measures the growth rate down a column against along a diagonal, and computes the predicted constants from the symbol. 615 cells, 0 failures; `dia/col = 1.414214` against `√2 = 1.414214`.
- `t20_selfsimilar.py` — is the scale family 2 → 4 → 8 → 16 self-similar? Three checks: whether block-sum and difference commute, whether the coarse table is a scalar multiple of the fine one at matched cells, and whether the depth profiles coincide after normalising each row's RMS by its depth-0 RMS. Not self-similar in the oscillation; similar in the trend under a depth rescaling by `1/ln((b−1)/b)`.
- `t21_fold_identify.py` — identifies the survivors of `t17`'s subtraction. Folds every zero into base 1.1175405's visible band by `γ ↦ 2·nyq − γ` and matches against the observed survivors; also asks whether the small survivors are differences of folded frequencies, reporting the candidate-pair count so a loose fit reads as the selection it is. γ₄ and γ₅ land on 26.114 and 23.602.
