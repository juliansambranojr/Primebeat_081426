...
| 6 | superseded — `the_container` repo was the earlier attempt at this CLI; `lab` phases 0–5 already cover its check, gate, seal, and chain plumbing; the map visualization is a separate project that reads from units/ | nothing to harvest; the program already covers the plumbing |
| 7 | retire the hook fleet down to 2 hooks: removed `check_direct_run` and `check_bash_guard` from PreToolUse/Bash (lab run enforces direct-run; commit gate covers bash-guard); removed `gate.py` from PostToolUse (commit gate catches broken refs); deleted 3 deregistered hook files from disk (`check_numbers_in_response`, `check_agent_brief`, `check_read_range`); left `check_response_prefix` (Stop) and `check_protected_write` (PreToolUse/Edit\|Write\|MultiEdit) as the surviving pair | the hook fleet is the two guards that protect the envelope: response prefix and write protection; everything else is covered by `lab` or the commit gate |
| 8 | the quote gate: a Stop hook that treats every blockquote line and fenced block in the assistant's message as a claimed quote and requires each to appear as an exact substring of the current session transcript; truncation allowed with ellipsis, every surviving segment still matches exactly; whitespace inside a copied table compared as written; described in § Transcript is king | a corrupted quote cannot pass; the relay rule becomes enforceable |
