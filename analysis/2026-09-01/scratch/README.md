...
| `mod_smoke.json` | 2026-09-02 10:10:30 | no header; JSON output of `analysis/2026-09-01/weil_Lc_mod.py` (self-declared `status: "EXPLORATORY - no prereg, no decision rule, no verdict"`), smoke run with ks = [1, 1000], eps = [0.0, 0.01], M = 16, dps 40, L grid 0.300..3.000 (4 pts) |
| `mod_smoke.txt` | 2026-09-02 10:10:30 | the table `weil_Lc_mod.py` wrote alongside `mod_smoke.json`; its header records the JSON's sha256 `f91f2afeb0767ca3826100f0eff5cce1c9c21865093f6fd37f6c87658b561db6` and the scratchpad path the JSON was written to |
| `xcheck_k1.py` | 2026-09-02 10:11:37 | no header; cross-check of the k=1 modulated row against the Legendre form — loads `weil_Lc_mod.py` by path, takes `ladder["k=1\|eps=0.01\|M=16\|w=1/2"]` from a `mod_smoke.json` in the directory given as `sys.argv[1]`, recomputes λ_mod on the pencil, rebuilds the M=64 Legendre form at the same h, projects the modulated minimiser onto it, and prints the Z′ / tail / 2wT split for both |
| `mod_smoke2.json` | 2026-09-02 10:13:37 | no header; JSON output of `analysis/2026-09-01/weil_Lc_mod.py` (self-declared exploratory), second smoke run with ks = [1, 300], eps = [0.01], M = 16, dps 40, L grid 0.300..19.887 (8 pts) |
| `mod_smoke2.txt` | 2026-09-02 10:13:37 | the table `weil_Lc_mod.py` wrote alongside `mod_smoke2.json`; header records the JSON's sha256 `e2479454aafba1ac8670a6963a6a73f2a6989f482b46a44996da59c50ac4c171` |
